        ...
        if len(list_users_subscriptions) != 0:
            ...
            for row in list_users_subscriptions:
                ...
                if str(subscription_id) == '1' and cur_date_time.minute == 0:
                    ...
                    photo_data = (url_of_result_image, True, 'Это ежечасная рассылка лучшей девочки', None)
                ...
                #все остальные рассылки отправляются в 22:00
                elif cur_date_time.hour == 20 and cur_date_time.minute == 12:
                    ...
                    if str(subscription_id) == '2':
                        ...
                        text_data = (text, None, None, 0, 0)

                        last_msg_id = queries_to_bd.get_last_msg_id()

                        #если текст подписки не пустой (а он может быть пустым, например из-за того, что сегодня нет международного праздника
                        if text != None:
                            sending.main(MypyBot, subscription_chat_id, last_msg_id, text_data, None, None, None, None, None)
